...
public class VistaInterfaz {
    ...
    // Mantengo txtHora por compatibilidad con Controler (se actualizará desde los ComboBox)
    private final TextField txtHora = new TextField();

    // NUEVO: dos ComboBox visibles para hora y minuto
    private final ComboBox<String> cbHoraHora = new ComboBox<>();
    private final ComboBox<String> cbHoraMinuto = new ComboBox<>();
    ...
    public void construirVista() {
        ...
        // --- Preparar los ComboBox de hora y minuto ---
        cbHoraHora.setPrefWidth(80);
        cbHoraMinuto.setPrefWidth(80);

        cbHoraHora.getItems().clear();
        cbHoraMinuto.getItems().clear();

        for (int h = 0; h < 24; h++) {
            cbHoraHora.getItems().add(String.format("%02d", h));
        }
        for (int m = 0; m < 60; m++) {
            cbHoraMinuto.getItems().add(String.format("%02d", m));
        }

        // Mantener txtHora para compatibilidad; lo ocultamos del layout (pero Controler continúa usando getTxtHora())
        txtHora.setPromptText(configuracion.getString("view.timePrompt"));
        txtHora.setVisible(false);
        txtHora.setManaged(false);

        // Cuando cambien los combos actualizamos txtHora en formato HH:mm (o vaciamos si falta uno)
        cbHoraHora.valueProperty().addListener((obs, oldV, newV) -> actualizarTxtHoraDesdeCombos());
        cbHoraMinuto.valueProperty().addListener((obs, oldV, newV) -> actualizarTxtHoraDesdeCombos());

        HBox horaBox = new HBox(6, cbHoraHora, new Label(":"), cbHoraMinuto);
        horaBox.setPadding(new Insets(0, 0, 0, 0));
        ...
        VBox formulario = new VBox(12,
                ...
                lblDiaSemana, cbDia,
                lblHora, horaBox, // mostramos el HBox con los dos ComboBox
                btnBuscar,
                lblEstado
        );
    ...
    // Actualiza txtHora con el valor seleccionado en los combos, o lo deja vacío si la selección está incompleta
    private void actualizarTxtHoraDesdeCombos() {
        String hh = cbHoraHora.getValue();
        String mm = cbHoraMinuto.getValue();
        if (hh != null && mm != null) {
    ...
    public TextField getTxtHora() { return txtHora; } // mantenemos la firma para compatibilidad
